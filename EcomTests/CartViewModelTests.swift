...
final class CartViewModelTests: XCTestCase {
    ...
    /// Event-based wait: cooperatively yields the current task until `predicate`
    /// becomes true. No wall-clock sleep — each `Task.yield` is a scheduling event.
    @MainActor
    static func waitUntil(
        ...
        if !predicate() {
            XCTFail("waitUntil exhausted \(attempts) yields", file: file, line: line)
        }
    }
}
